...
<<<<<<< BEC_Groundstate_imt.cpp
        groundstate=split_step::S3imag(L, deltat, g, groundstate, bec::Potential<double, true>());
=======
        groundstate=split_step::S3imag(L, deltat, g, groundstate, bec::Potential<double, true, false>());
>>>>>>> 1.5
        mynorm = L_2_norm(groundstate.begin(), groundstate.end(), L);
        transform(groundstate.begin(), groundstate.end(), groundstate.begin(), bind2nd(divides<complex<double> >(), mynorm) );
    }
...
<<<<<<< BEC_Groundstate_imt.cpp
	...
	for(;t<maxt; t+=deltat, ++stepcounter){
		...
		data=split_step::S3(L, deltat, g, data, bec::Potential<double, true>() );
