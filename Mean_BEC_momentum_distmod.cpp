...
int main(int argc, char* argv[]){
	...
	for(;t<maxt; t+=deltat){
		...
		mynorm = L_2_norm(data.begin(), data.end(), L);
		transform(data.begin(), data.end(), data.begin(),bind2nd(divides<complex_type>(), mynorm) );
		mom=FourierTransform<fft::forward, fft::Estimate>(data);
		mynorm = L_2_norm(mom.begin(), mom.end(), 2.*M_PI/L);
		transform(mom.begin(), mom.end(), mom.begin(),bind2nd(divides<complex_type>(), mynorm) );
