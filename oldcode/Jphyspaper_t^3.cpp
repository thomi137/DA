...
int main(){
	...
	const complex<double> I(0,1);
 	...
 	//k (momentum space)
  	...
  	for(unsigned int i = 0; i < k.size(); ++i)
      		k[i] = (i<N/2.) ? double(i)/double(N):-double(N-i)/double(N);
	...
	for(double t = 0.;t<=50.;t+=deltat){
		...
		//kinetic term
   		...
   		fftw_plan kinetic_backtransform = fftw_plan_dft_1d(N, out, in, FFTW_BACKWARD, FFTW_ESTIMATE);
    		...
    		for(int i = 0; i < N; ++i){
    			double real_part = out[i][0];
			double imag_part = out[i][1];
        		complex<double> exp_fac = exp(-I*deltat*k[i]*k[i]/2.);
		...
		//kinetic term
   		...
   		fftw_plan kinetic_backtransform2 = fftw_plan_dft_1d(N, out, in, FFTW_BACKWARD, FFTW_ESTIMATE);
    		fftw_execute(kinetic_backtransform2);
    		fftw_destroy_plan(kinetic_backtransform2);
 	}
