...
namespace split_step{
	...
	inline std::vector<std::complex<double> > S3 (const T L, const T deltat, const
			T g, std::vector<std::complex<double> > data, bec::Potential<T, Lattice, Trap> pot,  const T omega=1.)
		{
    ...
    template<class T, bool Lattice, bool Trap>
    inline std::vector<std::complex<double> > S3imag (const T L, const T deltat, const
                                                  T g, std::vector<std::complex<double> > data, bec::Potential<T, Lattice, Trap> pot)
    {
        const double pi = 4.*atan(1.);
        double q = pi*40./L;
        std::complex<double>I(0,1);
